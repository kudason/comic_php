    ...
    if(isset($_POST['submit'])) {
		...
		if(!empty($image_name)) {
            ...
            elseif($image_size > 5000000) {
                ...
                if(!empty($pass)) {
                    $update = mysqli_query($conn, "UPDATE `user` SET id='$id', name='$name', password='$pass_hash', image='$image_name', role='$role' WHERE id=$id") or die('query failed!');
                    ...
                    $update = mysqli_query($conn, "UPDATE `user` SET id='$id', name='$name', image='$image_name', role='$role' WHERE id=$id") or die('query failed!');
            ...
            if(!empty($pass)) {
                $update = mysqli_query($conn, "UPDATE `user` SET id='$id', name='$name', password='$pass_hash', role='$role' WHERE id=$id") or die('query failed!');
                ...
                $update = mysqli_query($conn, "UPDATE `user` SET id='$id', name='$name', role='$role' WHERE id=$id") or die('query failed!');
